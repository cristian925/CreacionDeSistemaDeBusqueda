    ...
    $.fn.imageUploader = function (options) {
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
				...
				$.jAlert({
					'title': "Error",
					'content': `Sólo se admiten archivos con extensiones "${plugin.settings.extensions.join('", "')}"`,
					'theme': "red",
        ...
        let validateMIME = function (file) {
            ...
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
				...
				$.jAlert({
					'title': "Error",
					'content': `Sólo se admiten los tipos de archivo "${plugin.settings.mimes.join('", "')}"`,
					'theme': "red",
					'autofocus': '.jalert_accept',
					'btns': [
						{'text':'Aceptar', 'closeAlert':true, 'theme': 'red', 'class': 'jalert_accept'}]
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
				...
				$.jAlert({
					'title': "Error",
					'content': `Peso máximo de cada archivo ${plugin.settings.maxSize / 1024 / 1024}Mb`,
					'theme': "red",
					'autofocus': '.jalert_accept',
					'btns': [
						{'text':'Aceptar', 'closeAlert':true, 'theme': 'red', 'class': 'jalert_accept'}]
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
				...
				$.jAlert({
					'title': "Error",
					'content': `Sólo se pueden subir ${plugin.settings.maxFiles} imágenes`,
					'theme': "red",
					'autofocus': '.jalert_accept',
					'btns': [
						{'text':'Aceptar', 'closeAlert':true, 'theme': 'red', 'class': 'jalert_accept'}]
